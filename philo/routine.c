...
/*   Updated: 2024/07/09 00:16:07 by tguerran         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void *thread_routine(void *t)
{
    ...
    while (1)
    {
        pthread_mutex_lock(&philo->data->simulation_over_mutex);  // Verrouillage du mutex simulation_over
        simulation_over = philo->data->simulation_over;
        pthread_mutex_unlock(&philo->data->simulation_over_mutex);  // Déverrouillage du mutex simulation_over
        ...
        pthread_mutex_lock(&philo->data->simulation_mutex);  // Verrouillage du mutex simulation
        if (philo->data->number_of_times_each_philosopher_must_eat != -1 &&
            philo->data->check_meal >= philo->data->number_of_philosophers)
        {
            pthread_mutex_unlock(&philo->data->simulation_mutex);  // Déverrouillage du mutex simulation
            break;
        }
        pthread_mutex_unlock(&philo->data->simulation_mutex);  // Déverrouillage du mutex simulation
